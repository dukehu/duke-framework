...
public class FileReadUtils {
    ...
    public static InputStream getStreamByFileName(String fileName) throws IOException {
        ...
        if (fileName.startsWith("http")) {
            // 网络地址
            return HttpUtils.downFile(fileName);
        } else if (BasicFileUtils.isAbsFile(fileName)) {
            // 绝对路径
            Path path = Paths.get(fileName);
            return Files.newInputStream(path);
        } else if (fileName.startsWith("~")) {
            // 用户目录下的绝对路径文件
            fileName = BasicFileUtils.parseHomeDir2AbsDir(fileName);
    ...
    public static String getMagicNum(String file) {
        try (InputStream stream = FileReadUtils.getStreamByFileName(file)) {
